...
@Injectable()
export class AssignTasksExistDetailActivate implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot):
    ...
    return this.checkStoreAndGetDetail(id).pipe(
      switchMap(() => this.hasTask(id))
    );
  }

  hasTask(id: number): Observable<boolean> {
    return this.store
      .select(fromStore.getAssignTasksEntitiesState)
      .pipe(
  ...
  checkStoreAndGetDetail(id: string): Observable<boolean> {
    return this.store.select(fromStore.getAssignTasksEntitiesState).pipe(
      ...
      tap(isDetailExist => {
        if (!isDetailExist) {
          this.store.dispatch(new fromStore.LoadDetailAssignTasks(id));
        }
      }),
