...
@Injectable()
export class UserCustomerExistDetailActivate implements CanActivate {
  ...
  hasRole(id: number): Observable<boolean> {
    return this.store
      .select(fromStore.getManageUserEntitiesState)
      .pipe(
  ...
  checkStoreAndGetDetail(id: string): Observable<boolean> {
    return this.store.select(fromStore.getManageUserEntitiesState).pipe(
      ...
      tap(isDetailExist => {
        if (!isDetailExist) {
          this.store.dispatch(new fromStore.LoadDetailManageUser(id));
        }
      }),
