...
public final class RestResponseWrapper {
    ...
    public static <T> ResponseEntity<BodyResponseDTO<T>> getResponse(HttpStatus status, ApiResponseCode errorCode, MessageUtils messageUtil, Page<T> page) {
        String message = errorCode.getMessage();
        if (messageUtil != null) {
            message = messageUtil.populate(errorCode.getMessage());
        }
        BodyResponseDTO<T> bodyResponse = new BodyResponseDTO<>(errorCode.getCode(), message, page);
        return new ResponseEntity<>(bodyResponse, status);
    }
